...
**災害の種別ごとに手順が違う。** 自分が触るものから読む。

| 種別 | 前処理 | 文書 |
|---|---|---|
| 浸水（flood） | 点群CSV → 格子 → **PNGタイル**／グラフへ焼き込み | [浸水データ](flood-data.md) |
| 地震（quake） | SHP → **GPKG正規化** → **GeoJSON**／グラフへ焼き込み | [地震データ](quake-data.md) |
| 道路網（OSM） | pbf → walkフィルタ → グラフ | [浸水データ](flood-data.md#新スコープscope-tokyo-23ku-tama-shigaikaの構築) |

⚠️ **経路への焼き込みは種別ごとに分かれていない。** グラフを焼くコマンドが浸水と地震を
まとめてエッジ属性へ入れるので、片方だけ焼き直すことはできない。

一次データの取得（公式配布元からのダウンロード）は種別共通で
[一次データの取得](raw-data.md)にある。
...
[ローカル実行・検証runbook](../local-runbook.md#5-6-重みコスト表を変えたとき)にまとめた。
