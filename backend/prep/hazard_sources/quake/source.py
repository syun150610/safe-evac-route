...
出所と検証内容は docs/dev/05_チーム移行案.md §4-8。
...
COLUMNS = {"quake_rank_building": "建物_ラ", "quake_rank_fire": "火災_ラ",
           "quake_rank_total": "総合_ラ"}

SCENARIOS = [
    {"id": "total",    "label": "総合危険度",   "column": "quake_rank_total",
     "note": "建物倒壊と火災を合わせた総合。既定"},
    {"id": "building", "label": "建物倒壊危険度", "column": "quake_rank_building",
     "note": "揺れによる建物倒壊のランク"},
    {"id": "fire",     "label": "火災危険度",    "column": "quake_rank_fire",
     "note": "延焼のランク"},
]
...
def sample_ranks(coords, gpkg):
    ...
    print(f"  {os.path.basename(gpkg)}: {len(gdf):,}町丁目 / "
          f"{gdf['区市町村名'].nunique()}市区町村 / CRS {gdf.crs}")

    missing = [v for v in COLUMNS.values() if v not in gdf.columns]
    if missing:
        raise ValueError(f"{gpkg}: 期待する列が無い {missing} / 実際={list(gdf.columns)}")

    # サンプル点を GeoDataFrame にして sjoin。
    # 面積・距離を計算しないので、投影せず EPSG:4326 のまま within 判定してよい。
    pts = gpd.GeoDataFrame(geometry=gpd.points_from_xy(coords[:, 0], coords[:, 1]),
                           crs=4326)
    j = gpd.sjoin(pts, gdf[["geometry"] + list(COLUMNS.values())],
                  how="left", predicate="within")
    # 境界上の点が複数ポリゴンにヒットしうる。最初の1件を採る
    j = j[~j.index.duplicated(keep="first")].sort_index()

    ranks = {k: pd.to_numeric(j[v], errors="coerce").to_numpy(np.float64)
             for k, v in COLUMNS.items()}
    covered = ~np.isnan(ranks["quake_rank_total"])
    n_out = int((~covered).sum())
    print(f"  サンプル点 {len(coords):,} のうち町丁目に載らなかった点: "
