"""浸水深CSVを共通格子へ載せ、座標から深さ・被覆を引く。

`load_grid()` はタイル生成と道路グラフ構築の両方が使う単一の実装である。
格子原点・被覆・値の扱いを変えた場合は、表示タイルとグラフを必ず同時に再生成する。
"""
...
# 単一流域どうしを比較表示するための最低ハッチ範囲。現在値は sumidagawa と
# kandagawa の合併bbox。render側が各シナリオの実データ範囲との和集合を取るため、
# より広い envelope の描画範囲をここで切り落とすことはない。
# 単一流域シナリオを追加して比較範囲を広げる場合は更新する。
...
def sniff_encoding(path):
    ...
    建設局の公式17 CSVにはUTF-8 BOM付きとCP932が混在するため、毎回判定する。
    """
...
def read_points(path):
    ...
    配布元の更新や互換入力で列順が変わっても壊れないよう、列は必ず名前で参照する。
    """
    name = os.path.basename(path)
    if name in SUSPECT_DATUM:
        print(
            f"  ! 警告: {name} は旧日本測地系の疑いがある"
            "（移行前データの検証結果。現在の建設局シナリオでは使用しない）"
        )
