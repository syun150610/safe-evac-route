...
def resolve_path_edges(G, path, weight):
    ...
    **探索に使ったのと同じ weight で argmin を取る**のが厳密な復元になる。
    ...
    edges, ambiguous = [], 0
    # 同じ列をずらしただけなので長さは必ず一致する。strict はその明示
...
def binding_segments(G, edges, depth):
    ...
    return [
        (u, v, k) for u, v, k in edges if float(G[u][v][k]["depth_max"]) >= depth - 1e-9
    ]
        ...
        feats.append(
            feature(
                edge_coords(G, u, v, k),
                {
                    ...
                    "hazard_cost": (
                        None
                        if math.isinf(float(d["hazard_cost"]))
...
def _km(lat1, lon1, lat2, lon2):
    return math.hypot(
        (lon1 - lon2) * 111.320 * math.cos(math.radians(lat1)), (lat1 - lat2) * 111.320
    )
...
def main():
    ...
    with open(args.graph, "rb") as f:
        G = pickle.load(f)
    print(
        f"graph: {args.graph}  nodes={G.number_of_nodes():,} edges={G.number_of_edges():,}"
    )
    ...
    for name, w in (("baseline", "length"), ("hazard", "weight_hazard")):
        ...
        if ambiguous:
            print(
                f"   ! 同weight・異depth の平行エッジ {ambiguous}箇所（key昇順で決定的に選択）"
            )
        if st["n_impassable_edges"]:
            print(
                f"   ! 通行不可(depth>=1.0m)エッジを {st['n_impassable_edges']}本 含む"
            )
    ...
    print(
        f"浸水域(>0.3m)通過率: {base['ratio_over_03'] * 100:.1f}% -> {haz['ratio_over_03'] * 100:.1f}%"
    )
    ...
    fc = {
        ...
        "features": [
            feature(
                results["baseline"]["coords"],
                {"route": "baseline", "label": "単純最短（距離のみ）", **base},
            ),
            feature(
                results["hazard"]["coords"],
    ...
    print(
        f"    その制約下での最短経路: {mm_stats['distance_m']:.0f}m "
    ...
    if verdict_key == "bottleneck":
        print(
            f"  => 判定: 迂回不能。hazard経路の max_depth={haz['max_depth_m']:.2f}m は"
        )
        print(
            f"     どう迂回しても避けられない下限（{thr:.2f}m）に達している。係数不足ではない。"
        )
    else:
        print(
            f"  => 判定: 迂回余地あり。max_depth を {thr:.2f}m まで下げる経路は存在するが、"
        )
        print(
            f"     hazard経路は {haz['max_depth_m']:.2f}m を選んだ。距離との釣り合いの問題。"
        )
    ...
    print(
        f"  hazard経路上で max_depth({haz['max_depth_m']:.2f}m) を作っている区間: {len(bind)}本"
    )
    for u, v, k in bind[:10]:
        ...
        frac = (
            sum(
                float(G[a][b][c]["length"])
                for a, b, c in results["hazard"]["edges"][:seq]
            )
            / haz["distance_m"]
        )
        nm = _name_of(e) or "(名称なし)"
        print(
            f"    - {nm}  len={float(e['length']):.0f}m  depth={float(e['depth_max']):.2f}m"
    ...
    afeats.append(
        feature(
            stitch(G, mm_edges),
            {
                "kind": "minimax",
                "route": "minimax",
                "label": f"最大浸水深を最小化した経路 (max={thr:.2f}m)",
                **mm_stats,
            },
        )
    )
    # ボトルネック: hazard経路の最深区間と、minimax経路の下限を作っている区間
    bfeats = bottleneck_features(
        G,
        results["hazard"]["edges"],
        haz["max_depth_m"],
        "hazard",
        o_ll,
        d_ll,
