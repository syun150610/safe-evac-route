...
# ⚠️ **単一種別のラベルは「◯◯を考慮」にする**（2026-08-23）。「地震のみ」は
#    「地震しか起きない経路」と読めてしまう。実際は「地震だけを重みに掛けた経路」。
# ⚠️ **掛け合わせは「浸水×地震」のまま**（同日、ユーザー判断）。
#    「浸水と地震を考慮」へは変えない。
# ⚠️ 番号（①②④⑤）は**画面には出していない**（色の見本で線と対応づく）。
#    ここに残しているのは、プリセットJSONの形を変えないため（決定 D-301）。
ROUTES = [
    (
        "baseline",
        "①",
...
def build(G, scenario, graph_file, o_name, d_name, note, with_segments=True):
    ...
    return {
        ...
        "od": {
            "origin": {
                "name": o_name,
                "display": _display(o_name),
                "latlon": list(o_ll),
                "node": int(o),
                "snap_m": round(snap_m(G, o, *o_ll), 1),
            },
            "dest": {
                "name": d_name,
                "display": _display(d_name),
                "latlon": list(d_ll),
                "node": int(d),
                "snap_m": round(snap_m(G, d, *d_ll), 1),
            },
            "note": note,
...
def main():
    ...
    index = {
        ...
        "default_od": next(
            (r["slug"] for r in od_index if r["role"] == "main"),
