...
from prep.hazard_sources.flood.cost import (COVERAGE_PENALTY, IMPASSABLE_FINITE,
                                            hazard_cost, hazard_cost_vec, _selftest_cost)
from prep.hazard_sources.quake.cost import (QUAKE_COST, QUAKE_COVERAGE_PENALTY,
                                            QUAKE_GPKG)
from prep.hazard_sources.flood.scenarios import SCENARIOS
from prep.paths import (CACHE_DIR as CACHE_DIR_PATH, data_path, graph_path,
                        quake_gpkg, rel)
# 「焼いたグラフを使う側」が要るものは snap.py に置いてある（osmnx 非依存）。
# ここから再エクスポートするので、既存の import 先は変えなくてよい
from prep.route_search.snap import (bbox_from_points, nearest_node,  # noqa: F401
                                    DEST_DEFAULT, MARGIN_KM, ORIGIN_DEFAULT,
                                    OUT_DEFAULT, snap_m)
...
def edge_geometries(G):
    ...
    for u, v, k, d in G.edges(keys=True, data=True):
        geom = d.get("geometry")
        if geom is None:
            geom = LineString([(nodes[u]["x"], nodes[u]["y"]),
                               (nodes[v]["x"], nodes[v]["y"])])
        keys.append((u, v, k))
        lines.append(geom)
...
def sample_points(lines, lengths, step_m: float):
    ...
    """
    counts = np.clip(np.floor(lengths / step_m).astype(np.int64) + 1, 2, MAX_SAMPLES_PER_EDGE)
...
def bake_hazard(G, grid, meta, step_m: float):
    ...
    if n_nocov:
        print(f"  ! {n_nocov:,} sample points ({n_nocov / len(depth) * 100:.2f}%) は"
              f"このシナリオの想定範囲外（グリッド外 {n_outside:,} を含む）")
        print(f"  !   → depth は 0.0m として扱うが、edge属性 coverage で識別できる")
    ...
    for i, (u, v, k) in enumerate(keys):
        ...
        d["hazard_cost_eff"] = float(cost_eff[i])   # 想定範囲外ペナルティ込み。探索はこちら
...
def bake_quake(G, keys, coords, offsets, counts, gpkg=QUAKE_GPKG):
    ...
    浸水コストとの合成は C-3 で行う。
    """
    from prep.hazard_sources.quake.source import COLUMNS as cols, sample_ranks
...
def check_connectivity(G, orig_node, dest_node):
    ...
    for u, v, k, d in G.edges(keys=True, data=True):
        if not d["impassable"]:
            P.add_edge(u, v, k, **d)
    ok = nx.has_path(P, orig_node, dest_node) if (orig_node in P and dest_node in P) else False
    return ok, P
...
def main():
    ap = argparse.ArgumentParser(description="ハザード付き歩行者グラフの構築（タスクA）")
    ...
    ap.add_argument("--margin-km", type=float, default=MARGIN_KM, help="bboxの片側マージン(km)")
    ap.add_argument("--sample-m", type=float, default=SAMPLE_M, help="エッジ上サンプル間隔(m)")
    ap.add_argument("--network-type", default="walk", help="OSMnx network_type")
    ap.add_argument("--rebuild", action="store_true", help="OSMキャッシュを無視して取り直す")
    ap.add_argument("--graphml", action="store_true", help="GraphMLも併せて出力")
    ap.add_argument("--skip-spec-check", action="store_true",
    ...
    is_reference = (len(csvs) == 1 and os.path.basename(csvs[0]) ==
                    os.path.basename(CSV_DEFAULT))
    if not is_reference:
    ...
    print(f"  grid {grid.shape}  max={grid.max():.2f}m  "
    ...
    print(f"  impassable edges (>=1.0m, cost=inf): {n_impassable:,} / {len(keys):,} "
    ...
    if ok:
        ...
        print(f"  ! WARNING: 通行不可(inf)エッジを除くと起終点が分断される。")
        print(f"  ! weight_hazard は inf を {IMPASSABLE_FINITE:.0e} 倍の有限値に置換済みなので")
        print(f"  ! 探索自体は成功するが、結果の経路は浸水1.0m超区間を含みうる。")
    ...
    meta_out = {
        ...
        "origin_latlon": list(origin), "dest_latlon": list(dest),
        "origin_node": int(orig_node), "dest_node": int(dest_node),
        ...
        "hazard_cost_bins": {"<0.10": 1.0, "<0.20": 1.3, "<0.30": 2.0,
                             "<0.50": 4.0, "<1.00": 12.0, ">=1.00": "inf"},
    ...
    if not args.skip_spec_check:
        ...
        if diffs:
            print(f"  ! SPECの記載と食い違い: {diffs} — データ更新の可能性あり。報告すること。")
        else:
